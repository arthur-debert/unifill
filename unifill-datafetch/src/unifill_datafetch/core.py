"""
Core functionality for downloading and processing Unicode character data.

This module contains the business logic for:
1. Downloading Unicode data files
2. Parsing and processing the data
3. Generating output in various formats
"""
...
def process_unicode_data(format_type='csv', output_dir='.', use_cache=False, cache_dir='./cache'):
    """
    Main function to process Unicode data and generate output files.
    
    Args:
        format_type: Output format (csv, json, lua, txt, or all)
        output_dir: Directory to write output files
        use_cache: Whether to use cached files
        cache_dir: Directory for cached files
        
    Returns:
        A tuple of (success, output_files) where success is a boolean indicating
        if the operation was successful, and output_files is a list of generated file paths.
    """
    # Download files to temporary location
    unicode_data_file = download_file(UNICODE_DATA_FILE_URL, use_cache, cache_dir)
    name_aliases_file = download_file(NAME_ALIASES_FILE_URL, use_cache, cache_dir)
    names_list_file = download_file(NAMES_LIST_FILE_URL, use_cache, cache_dir)
    cldr_annotations_file = download_file(CLDR_ANNOTATIONS_URL, use_cache, cache_dir)
    ...
    try:
        ...
        for fmt in formats:
            output_filename = os.path.join(output_dir, OUTPUT_FILES[fmt])
            if fmt == 'csv':
        ...
        if unicode_data_file:
            import shutil
            shutil.copy(unicode_data_file, os.path.join(output_dir, "UnicodeData.txt"))
            print(f"Saved Unicode data file to {os.path.join(output_dir, 'UnicodeData.txt')}")
        if name_aliases_file:
            import shutil
            shutil.copy(name_aliases_file, os.path.join(output_dir, "NameAliases.txt"))
            print(f"Saved name aliases file to {os.path.join(output_dir, 'NameAliases.txt')}")
        if names_list_file:
            import shutil
            shutil.copy(names_list_file, os.path.join(output_dir, "NamesList.txt"))
