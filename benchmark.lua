...
-- This script loads all backends and measures their performance
...
local function benchmark_standard_backend(backend_name, search_terms)
    ...
    -- Store results
    benchmark_results[backend_name].load_time = load_time_ms
    benchmark_results[backend_name].entries = #data
    ...
    for _, terms in ipairs(search_terms) do
        ...
        -- Store results
        table.insert(benchmark_results[backend_name].searches, {
            terms = term_str,
            matches = matches,
...
for _, search in ipairs(benchmark_results.csv.searches) do
    table.insert(csv_search_times, search.time)
end

for _, search in ipairs(benchmark_results.grep.searches) do
...
-- Lua backend row
local lua_row = {"lua", string.format("%7.2f ms", benchmark_results.lua.load_time), benchmark_results.lua.entries}
for _, term in ipairs(search_terms_list) do
...
-- CSV backend row
local csv_row = {"csv", string.format("%7.2f ms", benchmark_results.csv.load_time), benchmark_results.csv.entries}
for _, term in ipairs(search_terms_list) do
...
-- Grep backend row
local grep_row = {"grep", string.format("%7.2f ms", benchmark_results.grep.init_time), "N/A"}
for _, term in ipairs(search_terms_list) do
